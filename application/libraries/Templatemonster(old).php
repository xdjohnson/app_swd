...
class Templatemonster {
	...
	private
		...
		$page; # Added for debugging purposes on 26MAR2015
	...
	# PUBLIC FUNCTIONS
	public function page_to_string()
	{
		return $this->page;
	}
	...
	public function get_categories(){
		...
		$this->page = $page;  # Added for debugging purposes on 26MAR2015
	...
	private function _get_categories( $page ){
		...
		# get raw section to parse categories from
		preg_match( "/all-categories(.*?)<\/section>/s", $page, $raw_section );

		if ( ! isset( $raw_section[1] ) ){
		...
		# parse the anchor data from the raw section
		preg_match_all( "/<div><a[^>]+href=\"([^\"]+)\">([^<]+)<\/a>(.*?)<\/ul><\/div>/si", $raw_section[1], $raw_data );

		$length = count( $raw_data[2] );
		for ( $i = 0; $i < $length; $i++ ){

			preg_match_all( "/<a[^>]+href=\"([^\"]+)\">([^<]+)</s", $raw_data[3][$i], $sub_heading_data );
			$retVal[html_entity_decode(trim($raw_data[2][$i]))]['url'] = $raw_data[1][$i];
			$retVal[html_entity_decode(trim($raw_data[2][$i]))]['sub_headings'] = $this->get_sub_headings( $sub_heading_data );
